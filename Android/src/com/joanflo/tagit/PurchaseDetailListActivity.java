...
public class PurchaseDetailListActivity extends BaseActivity implements Button.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        viewingCart = pos != 0;
        Purchase purchase;
        if (viewingCart) { // view cart
        	// get purchase
        	purchase = user.getPurchaseCart();
        } else {
        	// view finished purchase detail
        	super.updateSelected(pos);
        	// get purchase
        	int purchaseId = bundle.getInt("purchaseId");
        	purchase = user.searchPurchaseById(purchaseId);
        }
	    
        preparePurchaseInfoSection(bundle, purchase);
	...
	private void prepareList() {
        ...
        while(it.hasNext()) {
        	...
        	// get product price
        	Country country = purchaseDetail.getBatch().getShop().getCity().getRegion().getCountry();
	...
	private void removeItem(int position) {
		...
		purchaseDetailItems.remove(position);
